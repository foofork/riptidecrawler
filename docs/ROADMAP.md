...
### 0.4. .env Variable Conflicts with API Request Parameters
**Status:** 🟡 MEDIUM
**Priority:** P1 - Causes confusion and override issues
**Estimated Effort:** 1 hour

**Description:**
`.env.example` contains request-level parameters (SPIDER_MAX_DEPTH, SPIDER_MAX_PAGES, SPIDER_CONCURRENCY, etc.) that conflict with API request parameters. These should be API request params, not system-level env vars.
...
# User's API request:
{
  ...
  "use_spider": true,
  "max_depth": 10,  # ← Does this override .env SPIDER_MAX_DEPTH=3?
  "max_pages": 500  # ← Confusing precedence!
}
```

**Impact:**
- User confusion about parameter precedence
- API requests may be overridden by .env defaults
- Harder to configure per-request behavior
- Documentation ambiguity

**Solution:**
1. **System-level config** (keep in .env): `SPIDER_ENABLE=true`, `REDIS_URL`, `HEADLESS_URL`
2. **Request-level params** (remove from .env): `SPIDER_MAX_DEPTH`, `SPIDER_MAX_PAGES`, `SPIDER_CONCURRENCY`, etc.

**Variables to Review:**
- [ ] SPIDER_MAX_DEPTH (request-level, remove from .env)
- [ ] SPIDER_MAX_PAGES (request-level, remove from .env)
- [ ] SPIDER_CONCURRENCY (request-level, remove from .env)
- [ ] SPIDER_TIMEOUT_SECONDS (request-level, remove from .env)
- [ ] SPIDER_DELAY_MS (request-level, remove from .env)
- [ ] SPIDER_BASE_URL (system-level, keep in .env)
- [ ] SPIDER_RESPECT_ROBOTS (request-level, remove from .env)
- [ ] ENHANCED_PIPELINE_FETCH_TIMEOUT (request-level?)
- [ ] ENHANCED_PIPELINE_RENDER_TIMEOUT (request-level?)

**Tasks:**
- [ ] Audit all .env.example variables for request vs system level
- [ ] Remove or comment out request-level params with warnings
- [ ] Document precedence rules (API request > .env defaults)
- [ ] Update API documentation with parameter sources
- [ ] Test API request overrides work correctly
