...
use riptide_core::html_parser::EnhancedHtmlExtractor;
...
async fn test_rust_lang_org_extraction() {
    ...
    assert!(result.is_ok(), "Failed to extract rust-lang.org: {:?}", result.err());

    let extraction = result.unwrap();
    assert!(extraction.extracted_chars > 100, "Extracted content too small");
    assert!(extraction.quality_score > 0.0, "Quality score should be > 0");
    println!("Rust-lang.org: {} chars, coverage: {:.2}%, quality: {:.2}",
...
async fn test_mdn_extraction() {
    ...
    assert!(extraction.extracted_chars > 100, "Extracted content too small");
    assert!(extraction.quality_score > 0.0, "Quality score should be > 0");
    println!("MDN: {} chars, coverage: {:.2}%, quality: {:.2}",
             extraction.extracted_chars, extraction.coverage_percent, extraction.quality_score);
}
...
async fn test_wikipedia_extraction() {
    ...
    assert!(result.is_ok(), "Failed to extract Wikipedia: {:?}", result.err());

    let extraction = result.unwrap();
    assert!(extraction.extracted_chars > 500, "Wikipedia article should have substantial content");
    assert!(extraction.quality_score > 0.3, "Quality score should be reasonable");
    println!("Wikipedia: {} chars, coverage: {:.2}%, quality: {:.2}",
             extraction.extracted_chars, extraction.coverage_percent, extraction.quality_score);
}
...
async fn test_hackernews_extraction() {
    ...
    assert!(result.is_ok(), "Failed to extract HackerNews: {:?}", result.err());

    let extraction = result.unwrap();
    assert!(extraction.extracted_chars > 50, "Should extract some content from HN");
    println!("HackerNews: {} chars, coverage: {:.2}%, quality: {:.2}",
             extraction.extracted_chars, extraction.coverage_percent, extraction.quality_score);
}
...
async fn test_comprehensive_suite() {
    ...
    for url in test_urls {
        ...
        match fetch_and_extract(url).await {
            Ok(result) => {
                println!("  ✓ Success - {} chars, coverage: {:.2}%, quality: {:.2}, time: {}ms",
                         result.extracted_chars, result.coverage_percent,
                         result.quality_score, result.extraction_time_ms);
                results.push(result);
                passed += 1;
            }
            Err(e) => {
    ...
    std::fs::write("test-results/comprehensive_extraction_results.json", results_json).unwrap();
    ...
    if summary.passed_tests > 0 {
        let avg_coverage: f64 = summary.results.iter()
            .map(|r| r.coverage_percent)
            .sum::<f64>() / summary.passed_tests as f64;

        let avg_quality: f64 = summary.results.iter()
...
fn test_simple_html_extraction() {
    ...
    println!("Simple HTML: {} chars, quality: {:.2}",
             result.main_content.len(), result.quality_score);
}
...
fn test_complex_html_extraction() {
    ...
    assert_eq!(result.metadata.og_description, Some("OG Description".to_string()));

    // Should be detected as article
    assert!(result.is_article);
    assert!(result.quality_score > 0.5);

    println!("Complex HTML: {} chars, quality: {:.2}",
             result.main_content.len(), result.quality_score);
}
