...
mod css_extraction_tests {
    ...
    async fn test_basic_css_selectors() {
        ...
        // Note: The current API returns "OG Title" from meta property, not "Test Page"
        assert!(!result.title.is_empty());
        assert!(result.content.contains("First paragraph"));
        assert!(result.content.contains("Second paragraph"));
        // Summary might be in the optional field
        if let Some(summary) = result.summary {
            assert_eq!(summary, "This is a test page");
        }
    ...
    async fn test_meta_tag_extraction() {
        ...
        // The default selectors prefer og:title over h1
    ...
    async fn test_css_selector_confidence_scoring() {
        ...
        assert!(complete_confidence > 0.5);
        assert!(minimal_confidence < 0.5);
    }
    ...
    async fn test_css_selector_edge_cases() {
        ...
        let result = extract_default(empty_html, "https://example.com")
            .await
            .unwrap();
        assert!(!result.title.is_empty()); // Should have fallback title
        assert!(result.content.is_empty() || result.content.len() < 100);
...
mod regex_extraction_tests {
    use riptide_extraction::{RegexExtractor, RegexPattern};
    ...
    async fn test_email_extraction() {
        ...
        assert!(
            result.content.contains("support@example.com")
                || result.content.contains("sales@company.org")
        );
    }
    ...
    async fn test_phone_number_extraction() {
        ...
        assert!(
            result.content.contains("123-456-7890") || result.content.contains("(555) 123-4567")
        );
    }
    ...
    async fn test_url_extraction() {
        ...
        assert!(
            result.content.contains("https://www.example.com")
                || result.content.contains("http://test.org")
        );
    }
    ...
    async fn test_complex_regex_patterns() {
        ...
        assert!(result.extraction_confidence > 0.5); // Should be high due to required pattern match
    }
    ...
    async fn test_regex_html_stripping() {
        ...
        assert!(
            result.content.contains("This text should remain")
                || result.content.contains("Regular content")
        );
    }
...
mod performance_edge_case_tests {
    ...
    async fn test_large_html_performance() {
        ...
        assert!(result.content.len() > 1000); // Should extract substantial content
    ...
    async fn test_html_with_binary_content() {
        ...
        assert!(content.content.contains("Text before") || content.content.contains("Text after"));
    }
    ...
    async fn test_extraction_with_multiple_strategies() {
        ...
        assert_eq!(css_result.strategy_used, "css_json_enhanced");
        assert_eq!(css_result.title, "Multi-Strategy Test");

        // Test regex extraction
        let regex_patterns = default_patterns();
        let regex_extractor = RegexExtractor::new(&regex_patterns).unwrap();
        let regex_result = regex_extractor
            .extract(html, "https://example.com")
            .await
            .unwrap();
