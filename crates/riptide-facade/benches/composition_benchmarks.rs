...
fn benchmark_stream_overhead(c: &mut Criterion) {
    ...
    c.bench_function("baseline_1000", |b| {
        b.iter(|| {
            rt.block_on(async {
                black_box(baseline_stream(1000).await)
            })
        })
    });
    ...
    c.bench_function("boxed_1000", |b| {
        b.iter(|| {
            rt.block_on(async {
                black_box(boxed_stream(1000).await)
            })
        })
    });
    ...
    c.bench_function("composed_1000", |b| {
        b.iter(|| {
            rt.block_on(async {
                black_box(composed_stream(1000).await)
            })
        })
    });
}
...
fn benchmark_single_item(c: &mut Criterion) {
    ...
    c.bench_function("baseline_single", |b| {
        b.iter(|| {
            rt.block_on(async {
                black_box(baseline_stream(1).await)
            })
        })
    });
    ...
    c.bench_function("boxed_single", |b| {
        b.iter(|| {
            rt.block_on(async {
                black_box(boxed_stream(1).await)
            })
        })
    });
}
