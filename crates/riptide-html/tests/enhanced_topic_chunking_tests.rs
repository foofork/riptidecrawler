...
async fn test_enhanced_coherence_scoring() {
    ...
    println!("Enhanced coherence: {} chunks in {}ms", chunks.len(), duration.as_millis());

    // Should identify the three main topics (ML, climate, economics)
    assert!(chunks.len() >= 2 && chunks.len() <= 4,
            "Should detect 2-4 topic boundaries, found {}", chunks.len());

    // Should be fast
    assert!(duration.as_millis() < 50, "Enhanced scoring should be fast: {}ms", duration.as_millis());

    // Verify topic coherence by checking keywords
    let topic_keywords: Vec<Vec<String>> = chunks.iter()
        .map(|c| c.metadata.topic_keywords.clone())
        .collect();

    // First chunk should be about machine learning
    let ml_terms = ["machine", "learning", "neural", "deep", "algorithms", "training"];
    assert!(topic_keywords[0].iter().any(|k| ml_terms.contains(&k.as_str())),
            "First chunk should contain ML terms: {:?}", topic_keywords[0]);

    // Later chunks should contain different topic terms
    let all_keywords: std::collections::HashSet<String> = chunks.iter()
        .flat_map(|c| c.metadata.topic_keywords.iter().cloned())
        .collect();

    // Should have diverse vocabulary across chunks
    assert!(all_keywords.len() > 8, "Should extract diverse keywords: {:?}", all_keywords);
}
...
async fn test_hysteresis_valley_detection() {
    ...
    println!("Hysteresis detection: {} chunks in {}ms", chunks.len(), duration.as_millis());

    // Should detect stable boundaries without oscillation
    assert!(chunks.len() >= 2 && chunks.len() <= 4,
            "Hysteresis should produce stable boundaries: {} chunks", chunks.len());

    // Verify boundary stability by running multiple times
    for i in 0..5 {
        let test_chunks = chunker.chunk(text).await.unwrap();
        assert_eq!(chunks.len(), test_chunks.len(),
                  "Boundary detection should be deterministic (run {})", i);

        // Verify same boundaries
        for (j, (orig, test)) in chunks.iter().zip(test_chunks.iter()).enumerate() {
            assert_eq!(orig.content, test.content,
                      "Chunk {} content should be identical across runs", j);
        }
    }
}
...
async fn test_performance_optimizations() {
    ...
    for run in 0..5 {
        ...
        println!("Run {}: {} chunks in {}ms", run + 1, chunks.len(), duration.as_millis());

        // Should meet performance requirement
        assert!(duration.as_millis() < 200,
               "Performance optimization failed: {}ms > 200ms", duration.as_millis());
        ...
        for chunk in &chunks {
            ...
            assert!(!chunk.metadata.topic_keywords.is_empty(), "Chunks should have keywords");
        }
    }
    ...
    println!("Average duration: {}ms, Max duration: {}ms", avg_duration, max_duration);
    assert!(avg_duration < 150, "Average performance should be well under limit: {}ms", avg_duration);
}
...
async fn test_fallback_mechanism() {
    ...
    while large_text.len() < 160_000 { // Above 150KB threshold
    ...
    println!("Fallback test: {} chunks in {}ms", chunks.len(), duration.as_millis());

    // Should still meet performance requirement due to fallback
    assert!(duration.as_millis() < 200, "Fallback should ensure performance: {}ms", duration.as_millis());

    // Should produce chunks
    assert!(!chunks.is_empty(), "Should produce chunks even with fallback");
    ...
    if large_text.len() > 150_000 {
        assert!(fallback_used, "Should use fallback for very large documents");
    }
}
...
async fn test_prominence_filtering() {
    ...
    assert!(chunks.len() >= 2 && chunks.len() <= 4,
           "Prominence should filter weak boundaries: {} chunks", chunks.len());
    ...
    let has_deep_learning = all_keywords.iter().any(|k|
        k.contains("deep") || k.contains("neural") || k.contains("learning"));
    let has_quantum = all_keywords.iter().any(|k|
        k.contains("quantum") || k.contains("superposition"));
    let has_blockchain = all_keywords.iter().any(|k|
        k.contains("blockchain") || k.contains("cryptographic"));

    assert!(has_deep_learning, "Should detect deep learning topic");
    assert!(has_quantum, "Should detect quantum computing topic");

    if chunks.len() >= 3 {
        assert!(has_blockchain, "Should detect blockchain topic in separate chunk");
    }
}
...
async fn test_minimum_distance_filtering() {
    ...
    assert!(chunks.len() <= 4, "Should limit chunks due to minimum distance: {} chunks", chunks.len());

    // Each chunk should have reasonable size
    for (i, chunk) in chunks.iter().enumerate() {
        assert!(chunk.content.len() >= 50, "Chunk {} too small: {} chars", i, chunk.content.len());
        assert!(chunk.metadata.word_count >= 8, "Chunk {} has too few words: {}", i, chunk.metadata.word_count);
    }
}
...
async fn test_deterministic_boundaries() {
    ...
    for run in 0..10 {
        ...
        if run > 0 {
            ...
            assert_eq!(all_chunks[0].len(), all_chunks[run].len(),
                      "Number of chunks should be deterministic across runs");

            for (i, (chunk1, chunk2)) in all_chunks[0].iter().zip(all_chunks[run].iter()).enumerate() {
                assert_eq!(chunk1.content, chunk2.content,
                          "Chunk {} content should be identical across runs", i);
                assert_eq!(chunk1.start_pos, chunk2.start_pos,
                          "Chunk {} positions should be identical", i);
                assert_eq!(chunk1.end_pos, chunk2.end_pos,
                          "Chunk {} positions should be identical", i);
            }
        }
    }

    println!("Deterministic test: {} chunks consistently across 10 runs", all_chunks[0].len());
...
async fn test_keyword_extraction_quality() {
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        println!("Chunk {}: {} keywords: {:?}",
                i, chunk.metadata.topic_keywords.len(), chunk.metadata.topic_keywords);

        // Should extract meaningful keywords
        assert!(!chunk.metadata.topic_keywords.is_empty(),
               "Chunk {} should have keywords", i);
        assert!(chunk.metadata.topic_keywords.len() <= 5,
               "Should limit keywords to top 5: {}", chunk.metadata.topic_keywords.len());

        // Keywords should be relevant to content
        let content_lower = chunk.content.to_lowercase();
        for keyword in &chunk.metadata.topic_keywords {
            assert!(content_lower.contains(keyword),
                   "Keyword '{}' should appear in chunk content", keyword);
        }
    }
