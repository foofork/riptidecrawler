...
impl PyRipTide {
    ...
    fn new(api_key: Option<String>) -> PyResult<Self> {
        ...
        let runtime = Runtime::new()
            .map_err(|e| RipTideError::runtime_error(format!("Failed to create async runtime: {}", e)))?;

        // Create AppState
        let state = runtime.block_on(async {
            AppState::new()
                .await
                .map_err(|e| RipTideError::initialization_error(format!("Failed to create AppState: {}", e)))
        })?;
