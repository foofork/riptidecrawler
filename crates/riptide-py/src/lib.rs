...
fn test_async_basic() -> PyResult<String> {
    ...
    rt.block_on(async { Ok("Async runtime works!".to_string()) })
}
...
fn riptide(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add(
        "__doc__",
        "Riptide Python Bindings - High-performance web scraping for Python",
    )?;
...
mod tests {
    ...
    fn test_async_execution() {
        ...
        let result = rt.block_on(async { "success" });
