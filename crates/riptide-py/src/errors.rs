...
use pyo3::exceptions::{
    PyException, PyRuntimeError, PyTimeoutError, PyValueError,
};
use pyo3::prelude::*;
...
impl From<RipTideError> for PyErr {
    fn from(err: RipTideError) -> PyErr {
        match err.error_type {
            ...
            ErrorType::Network => PyRuntimeError::new_err(format!("Network error: {}", err.message)),
            ErrorType::Extraction => PyRuntimeError::new_err(format!("Extraction error: {}", err.message)),
            ErrorType::Initialization => PyRuntimeError::new_err(format!("Initialization error: {}", err.message)),
        }
    }
}
