...
impl PerformanceMonitor {
    ...
    /// Analyze CPU-bound bottlenecks
    async fn analyze_cpu_bottlenecks(
        &self,
    ...
    /// Analyze memory bottlenecks
    async fn analyze_memory_bottlenecks(
        &self,
        metrics: &VecDeque<SystemMetrics>
    ) -> Result<Vec<Bottleneck>> {
        let mut bottlenecks = Vec::new();
    ...
    /// Analyze I/O bottlenecks
    async fn analyze_io_bottlenecks(
        &self,
        metrics: &VecDeque<SystemMetrics>
    ) -> Result<Vec<Bottleneck>> {
        let mut bottlenecks = Vec::new();
    ...
    /// Analyze application-level bottlenecks
    async fn analyze_application_bottlenecks(
        &self,
            ...
            match bottleneck.location.as_str() {
                loc if loc.contains("CPU") => {
                    if bottleneck.severity == BottleneckSeverity::Critical {
                        ...
                        recommendations.push(format!(
                            "HIGH: CPU usage elevated - Investigate hot paths and consider async processing for CPU-intensive tasks"
                        ));
                    }
                },
