...
pub fn is_pdf_content(content_type: Option<&str>, data: &[u8]) -> bool {
    ...
    detect_pdf_by_magic_bytes(data)
}

/// Detect PDF by magic bytes only
pub fn detect_pdf_by_magic_bytes(data: &[u8]) -> bool {
    if data.len() < 5 {
        return false;
    }
...
/// Detect PDF by file extension
pub fn detect_pdf_by_extension(url_or_path: &str) -> bool {
    url_or_path.to_lowercase().ends_with(".pdf")
}

/// Comprehensive PDF detection combining multiple methods
pub fn detect_pdf_content(
    ...
    // Priority 2: Magic bytes (most reliable for actual content)
    if let Some(data_bytes) = data {
        if detect_pdf_by_magic_bytes(data_bytes) {
            return true;
        }
    }

    // Priority 3: File extension (least reliable, but useful for URLs)
    if let Some(path) = url_or_path {
        if detect_pdf_by_extension(path) {
            return true;
        }
    }
...
mod tests {
    ...
    fn test_is_pdf_content() {
        ...
        assert!(!is_pdf_content(Some("text/html"), b"not a pdf"));
    }
    ...
    fn test_detect_pdf_by_extension() {
        assert!(detect_pdf_by_extension("document.pdf"));
        assert!(detect_pdf_by_extension("Document.PDF"));
        assert!(detect_pdf_by_extension("/path/to/file.pdf"));
        assert!(detect_pdf_by_extension("https://example.com/doc.pdf"));
    ...
    fn test_comprehensive_pdf_detection() {
        ...
        // Magic bytes should work
        assert!(detect_pdf_content(None, None, Some(pdf_data)));

        // Extension should work as fallback
        assert!(detect_pdf_content(None, Some("doc.pdf"), None));

        // Combined detection
        assert!(detect_pdf_content(
