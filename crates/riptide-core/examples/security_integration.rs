...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "✅ Created production key: {} ({})",
        production_key.name, production_key.id
    );
    println!(
        "✅ Created development key: {} ({})\n",
    ...
    match result {
        Ok(context) => {
            ...
            println!(
                "   - Payload redacted: {} chars -> {} chars",
                request_with_pii.len(),
                context.redacted_payload.len()
            );
            ...
            let response_result = security_middleware
                .process_response(
                    "job-001",
            ...
            println!(
                "   - Response redacted: {} chars -> {} chars",
                response_with_pii.len(),
                response_result.len()
            );
        }
        Err(e) => {
    ...
    println!(
        ...
        health.security_violations_today
    );
    ...
    println!(
        "   - Current Usage: ${:.2}",
        budget_health.current_usage_usd
    );
    println!(
        "   - Monthly Limit: ${:.2}",
        budget_health.monthly_limit_usd
    );
    println!(
        "   - Projected Monthly Cost: ${:.2}",
        budget_health.projected_monthly_cost
    );
    println!(
        "   - Days Remaining: {}",
        budget_health.days_remaining_in_month
    );
    println!(
        "   - Warning Threshold Reached: {}",
        budget_health.warning_threshold_reached
    );
