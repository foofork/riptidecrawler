// Re-export the modular monitoring system
pub mod monitoring;
...
impl MetricsCollector {
    ...
    /// Start the metrics collection background task with telemetry
    pub async fn start_collection(&self) {
        let _span = telemetry_span!("metrics_collection_started");
    ...
    pub async fn record_extraction(
        ...
        let _span = telemetry_span!(
            ...
            "was_cached" => was_cached.to_string()
        );
    ...
    /// Record error occurrence with telemetry
    ...
    async fn collect_system_metrics(&self) {
        ...
        let memory_usage = self.get_memory_usage();
        {
            match self.memory_usage.lock() {
                ...
                Err(e) => {
                    tracing::warn!("Failed to acquire memory metrics lock: {}", e);
                }
            }
        }
