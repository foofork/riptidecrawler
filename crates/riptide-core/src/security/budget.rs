...
impl BudgetUsage {
    ...
    pub fn add_usage(&mut self, tenant_id: &TenantId, cost_info: &CostInfo) {
        ...
        *self.model_costs.entry(cost_info.model_name.clone()).or_insert(0.0) += cost_info.estimated_cost_usd;
...
impl BudgetCircuitBreaker {
    ...
    pub fn record_failure(&mut self, grace_period_minutes: u32) {
        ...
        if self.state == CircuitBreakerState::Closed {
            ...
            self.grace_period_end = Some(Utc::now() + Duration::minutes(grace_period_minutes as i64));
...
impl BudgetManager {
    ...
    pub async fn check_budget_for_job(
        ...
        // Check circuit breaker first
        {
            let mut breaker = self.circuit_breaker.write()
                .map_err(|e| SecurityError::Unknown(format!("Circuit breaker lock error: {}", e)))?;
        ...
        let estimated_cost = self.calculate_estimated_cost(estimated_tokens, model_name).await?;
        ...
        if estimated_cost > self.limits.per_job_limit_usd {
            ...
            return Err(SecurityError::BudgetLimitExceeded(
                format!("Job cost ${:.2} exceeds per-job limit ${:.2}", 
        ...
        if projected_total > self.limits.global_monthly_limit_usd {
            ...
            self.record_budget_violation().await;
            return Err(SecurityError::BudgetLimitExceeded(
                format!("Total monthly cost would reach ${:.2}, exceeding limit ${:.2}",
        ...
        if let Some(tenant_limit) = self.limits.per_tenant_monthly_limit_usd {
            ...
            if projected_tenant_total > tenant_limit {
                ...
                self.record_budget_violation().await;
                return Err(SecurityError::BudgetLimitExceeded(
                    format!("Tenant monthly cost would reach ${:.2}, exceeding limit ${:.2}",
    ...
    async fn calculate_estimated_cost(&self, tokens: u64, model_name: &str) -> SecurityResult<f64> {
        let pricing = self.model_pricing.read()
        ...
        if let Some(model_pricing) = pricing.get(model_name) {
            ...
            warn!(model_name = model_name, "Unknown model, using default pricing");
    ...
    pub async fn check_budget_health(&self) -> BudgetHealthStatus {
        let usage = self.current_usage.read().await;
        let usage_percentage = (usage.total_cost_usd / self.limits.global_monthly_limit_usd) * 100.0;
    ...
    pub async fn update_model_pricing(&self, model_name: String, pricing: ModelPricing) -> Result<()> {
        let mut pricing_map = self.model_pricing.write()
...
mod tests {
    ...
    async fn test_per_job_limit_exceeded() {
        ...
        assert!(matches!(result.unwrap_err(), SecurityError::BudgetLimitExceeded(_)));
    }
